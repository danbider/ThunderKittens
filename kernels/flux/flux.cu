...
void flux_prepare(
    ...
    vec_smem_1xD (&img_s) [2][NUM_WORKERS] = al.allocate<vec_smem_1xD,2,NUM_WORKERS>();
    ...
    int n_blocks = IMG_D / NUM_WORKERS;
    for (int block = 0; block < n_blocks; block ++, tic ^=1, toc ^=1) {
        ...
        // save output
        store(o_g + (block*NUM_WORKERS +warpid)*D, residual_s[warpid][tic]); 
    }
}
