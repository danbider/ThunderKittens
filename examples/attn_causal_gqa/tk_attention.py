    ...
    def forward(
        ...
        **kwargs,
        ...
        if (query_states.shape[2] != key_states.shape[2]):
            # pre-pad sequence dimension q so that it is == k
            q_len_pad    = key_states.shape[2]
            query_states = torch.nn.functional.pad(query_states, (0, 0, q_len_pad - q_len, 0), value=0)
            decoding = True
        
        # pad seqlen to multiple of 128
        q_len_og = query_states.shape[2]
        if q_len_og % 128 != 0:
            q_len_pad    = q_len_og + (128 - (q_len_og % 128))
            
            query_states = torch.nn.functional.pad(query_states, (0, 0, 0, q_len_pad - q_len_og), value=0)
            key_states   = torch.nn.functional.pad(key_states, (0, 0, 0, q_len_pad - q_len_og), value=0)
            value_states = torch.nn.functional.pad(value_states, (0, 0, 0, q_len_pad - q_len_og), value=0)
        ...
        attn_output_2 = attn_output_2[:, :, :q_len_og, :]
        
        if decoding:
            attn_output_2 = attn_output_2[:, :, -1:, :]
        ##########
        ##########
        ...
        # attn_output_3 = self.o_proj(attn_output_3.float())
        ...
        # avg_diff_mag = torch.mean(torch.abs(attn_output - attn_output_3)).item()
